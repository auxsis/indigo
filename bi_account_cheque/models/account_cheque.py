...
class AccountCheque(models.Model):
    ...
    sequence = fields.Char(
        string='Sequence', readonly=True, copy=True, index=True)
    name = fields.Char(string="Name", required="1")
    bank_account_id = fields.Many2one('account.account', 'Bank Account')
    account_cheque_type = fields.Selection(
        [('incoming', 'Incoming'), ('outgoing', 'Outgoing')], string="Cheque Type")
    cheque_number = fields.Char(string="Cheque Number", required=True)
    amount = fields.Float(string="Amount", required=True)
    cheque_date = fields.Date(string="Cheque Date", default=datetime.now())
    cheque_given_date = fields.Date(
        string="Cheque Given Date")     # for outgoing
    cheque_receive_date = fields.Date(
        string="Cheque Receive Date")  # for incoming
    cheque_return_date = fields.Date(string="Cheque Return Date")
    payee_user_id = fields.Many2one(
        'res.partner', string="Payee", required="1")
    credit_account_id = fields.Many2one(
        'account.account', string="Credit Account")
    debit_account_id = fields.Many2one(
        'account.account', sring="Debit Account")
    comment = fields.Text(string="Comment")
    attchment_ids = fields.One2many(
        'ir.attachment', 'account_cheque_id', string="Attachment")
    status = fields.Selection([('draft', 'Draft'), ('registered', 'Registered'), ('bounced', 'Bounced'), ('return', 'Returned'), (
        'cashed', 'Done'), ('cancel', 'Cancel')], string="Status", default="draft", copy=False, index=True, track_visibility='onchange')

    status1 = fields.Selection([('draft', 'Draft'), ('registered', 'Registered'), ('bounced', 'Bounced'), ('return', 'Returned'), ('deposited', 'Deposited'), (
        'transfered', 'Transfered'), ('cashed', 'Done'), ('cancel', 'Cancel')], string="Status", default="draft", copy=False, index=True, track_visibility='onchange')
    ...
    journal_items_count = fields.Integer(
        compute='_active_journal_items', string="Journal Items")
    invoice_ids = fields.One2many(
        'account.invoice', 'account_cheque_id', string="Invoices", compute="_count_account_invoice")
    attachment_count = fields.Integer(
        'Attachments', compute='_get_attachment_count')
    ...
    def open_payment_matching_screen(self):
        ...
        #                 break;
        action_context = {'company_ids': [
            self.company_id.id], 'partner_ids': [self.payee_user_id.id]}
...
#             action_context.update({'move_line_id': move_line_id})
    ...
    def _active_journal_items(self):
        ...
        for journal_items in self:
            journal_item_ids = self.env['account.move'].search(
                [('account_cheque_id', '=', journal_items.id)])
    ...
    def create(self, vals):
        ...
        vals['sequence'] = sequence.with_context(
    ...
    def set_to_return(self):
        ...
        if len(matching_dict) != 0:
            ...
            a = self.env['account.move.line'].search(
        ...
        if self.account_cheque_type == 'incoming':
            ...
            account_move.write({'line_ids': move_lines})
            self.status1 = 'return'
            self.cheque_return_date = datetime.now()
        else:
            vals = {
                'name': self.name,
                'date': self.cheque_given_date,
...
class ChequeWizard(models.TransientModel):
    ...
    def default_get(self, flds):
        ...
        account_cheque_id = self.env['account.cheque'].browse(
    ...
    def create_cheque_entry(self):
        account_cheque = self.env['account.cheque'].browse(
...
class ReportWizard(models.TransientModel):
    ...
    cheque_type = fields.Selection(
        [('incoming', 'Incoming'), ('outgoing', 'Outgoing')], string="Cheque Type", default='incoming')
    #incoming_cheque = fields.Boolean('Incoming Cheque',default=True)
    #outgoing_cheque = fields.Boolean('Outgoing Cheque')
    ...
    def submit(self):
        ...
        if self.cheque_type == 'incoming':
            in_account_cheque_ids = self.env['account.cheque'].search(
                [('cheque_date', '>=', self.from_date), ('cheque_date', '<=', self.to_date), ('account_cheque_type', '=', 'incoming')])

            if not in_account_cheque_ids:
                raise UserError(_('There Is No Any Cheque Details.'))
            else:
                for inc in in_account_cheque_ids:
                    # inc_temp.append(inc.id)
                    temp.append(inc.id)

        if self.cheque_type == 'outgoing':
            out_account_cheque_ids = self.env['account.cheque'].search(
                [('cheque_date', '>=', self.from_date), ('cheque_date', '<=', self.to_date), ('account_cheque_type', '=', 'outgoing')])

            if not out_account_cheque_ids:
                raise UserError(_('There Is No Any Cheque Details.'))
            else:
                for out in out_account_cheque_ids:
                    # out_temp.append(out.id)
                    temp.append(out.id)
        ...
        datas = {
            ...
            # 'in_cheque' : self.incoming_cheque,
            # 'out_cheque' : self.outgoing_cheque,
            # 'in_data' : in_data,
            # 'out_data' : out_data
        }
#         return  self.env['report'].get_action(self, 'sales_commission_generic.sale_inv_comm_template', data=datas)
    ...
    def submit(self):
        ...
        if self.incoming_cheque == True:
            in_account_cheque_ids = self.env['account.cheque'].search([('cheque_date','>=',self.from_date),('cheque_date','<=',self.to_date),('account_cheque_type','=','incoming')])
            print ("innnnnnnnnnnnnnnnin_account_cheque_idsnnnnnnnnnnnnnn",in_account_cheque_ids)
            ...
            if not in_account_cheque_ids:
                ...
                for inc in in_account_cheque_ids:
                    inc_temp.append(inc.id)
                    #temp.append(inc.id)
                    print ("AAAAAAAAAAAAAAAAAAA",temp)
            
        if self.outgoing_cheque == True:
            out_account_cheque_ids = self.env['account.cheque'].search([('cheque_date','>=',self.from_date),('cheque_date','<=',self.to_date),('account_cheque_type','=','outgoing')])
            print ("outttttttout_account_cheque_idstttttttttttt",out_account_cheque_ids)
        ...
        print ("0000000000000000000",datas)
#         return  self.env['report'].get_action(self, 'sales_commission_generic.sale_inv_comm_template'''
