...
#     unreconciled_aml_ids = fields.One2many('account.move.line', 'partner_id', 'Move lines')
	...
	def compute_days_filter(self):
		...
		for partner in self:
			...
			for ml in move_lines :
				diff = from_date - datetime.strptime(ml.date_maturity,'%Y-%m-%d').date() 
	...
	def compute_days(self):
		...
		for partner in self:
			...
			move_lines = self.env['account.move.line'].search(domain) 
			for ml in move_lines :
				diff = today - datetime.strptime(ml.date_maturity,'%Y-%m-%d').date() 
				if diff.days >= 0 and diff.days <= 30:
		...
		for partner in self:
			...
			if partners_to_email and partner.payment_amount_overdue_amt != 0:
				...
				if partner not in partners_to_email:
					msg = _('Overdue email sent to %s' % ', '.join(['%s <%s>' % (partner.name, partner.email) for partner in partners_to_email]))
		...
		for partner in self:
			...
			if partners_to_email:
				...
				if partner not in partner_to_email:
					msg = _('Customer Monthly Statement email sent to %s' % ', '.join(['%s <%s>' % (partner.name, partner.email) for partner in partners_to_email]))
					partner.message_post(body=msg)
		return unknown_mails
