...
class CustomerStatementMonthly(models.TransientModel):
    ...
    year = fields.Selection([(num, str(num)) for num in range(
        1999, (datetime.now().year)+1)], string='Year', required=True)
...
#     date_upto = fields.Date('Upto Date', required="1",default=fields.Datetime.now)
    ...
    def onchange_month_year(self):
        if self.month and self.year:
            ...
            date_to = datetime(year=self.year, month=month, day=1) + \
                timedelta(days=calendar.monthrange(self.year, month)[1] - 1)
