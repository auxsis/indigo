        ...
        $BODY$
                BEGIN
                    ...
                    Select 
                        ...
                        round((final_data.overstock_qty * final_data.stock_value)::numeric,0) ovestock_value, final_data.purchase_date, 
                    ...
                    (
                        ...
                        (
                            Select
                                ...
                                po_line.product_qty as purchase_qty,
                    ...
                    (
                        ...
                        From 
                            stock_move move
                                ...
                                Inner Join product_template tmpl on tmpl.id = prod.product_tmpl_id 
                                Inner Join product_category cat on cat.id = tmpl.categ_id
                                Left Join stock_warehouse source_warehouse ON source.id = source_warehouse.lot_stock_id
                                Left Join stock_warehouse dest_warehouse ON dest.id = dest_warehouse.lot_stock_id
                ...
                (
                    ...
                    from 
                        stock_quant quant
                            ...
                            Inner Join product_template tmpl on tmpl.id = prod.product_tmpl_id
                            Left Join stock_warehouse ware ON loc.id = ware.lot_stock_id
                    ...
                    from 
                        stock_move move
                            ...
                            Inner Join product_product prod on prod.id = move.product_id
                            Inner Join product_template tmpl on tmpl.id = prod.product_tmpl_id
                            Left Join stock_warehouse source_ware ON source.id = source_ware.lot_stock_id
                            Left Join stock_warehouse dest_ware ON dest_location.id = dest_ware.lot_stock_id
                    Where 
...
def create_get_stock_age_data_sp(cr):
    ...
    DROP FUNCTION IF EXISTS public.inventory_stock_age_report(integer[], integer[], integer[]);
        ...
        BEGIN
                ...
                Select 
                        row_number() over(partition by move.company_id, move.product_id order by move.company_id, move.product_id, move.date::date) row_id,
                        move.company_id, move.company_name, move.date::date, move.product_id, move.product_name,
                        move.product_category_id, move.category_name, quant.current_stock, quant.current_stock_value,move.stock_value, move.stock_qty
                from
                (
                select 
                    --row_number() over(partition by move.company_id, move.product_id order by move.company_id, move.product_id, move.date::date) row_id,
                    ...
                    tmpl.categ_id as product_category_id,
                    cat.name as category_name,
                    sum(move.remaining_qty) stock_qty,
                    sum(move.remaining_value) stock_value 
                from stock_move move
                    Inner Join res_company cmp on cmp.id = move.company_id
                    Inner Join product_product prod on prod.id = move.product_id
                    Inner Join product_template tmpl on tmpl.id = prod.product_tmpl_id
                    Inner Join product_category cat on cat.id = tmpl.categ_id
                Where remaining_qty > 0 and prod.active = True and tmpl.active = True and tmpl.type = 'product' and move.state='done'
                     and 1 = case when array_length(company_ids,1) >= 1 then 
                    case when move.company_id = ANY(company_ids) then 1 else 0 end
                    else 1 end
                     --product dynamic condition
                ...
                group by move.company_id, cmp.name, move.product_id, coalesce(prod.default_code, tmpl.name), tmpl.categ_id, cat.name, move.date::date
                ) move
                Inner Join 
                (
                    Select q.company_id,q.product_id, round((sum(q.quantity) * max(COALESCE(prop.value_float,0)))::numeric,2) as current_stock_value, sum(q.quantity) current_stock
                    from stock_quant q
                        inner join stock_location loc on loc.id = q.location_id
                        left join ir_property prop on prop.res_id = 'product.product,' || q.product_id and prop.name='standard_price'
                ...
                on quant.product_id = move.product_id and quant.company_id = move.company_id;
                
                Return Query
                        with all_data as (
                            Select st.company_id, st.company_name, st.product_id, st.product_name, st.product_category_id, st.category_name, 
                                max(st.current_stock) as total_stock, max(st.current_stock_value) as total_stock_value
                        ...
                        oldest_stock_data as (
                            Select st.company_id, st.product_id, st.in_date, st.stock_qty, st.stock_value,	
                        ...
                        Select 
                            all_data.*, oldest.in_date, oldest.days_old, oldest.stock_qty, oldest.stock_value,
...
def create_get_stock_age_breakdown_data_sp(cr):
    query = """
      ...
      RETURNS TABLE(company_id integer, company_name character varying, product_id integer, product_name character varying, product_category_id integer, category_name character varying, total_stock double precision, total_stock_value double precision, breakdown1_qty double precision, breckdown1_value double precision, breakdown2_qty double precision, breckdown2_value double precision, breakdown3_qty double precision, breckdown3_value double precision, breakdown4_qty double precision, breckdown4_value double precision, breakdown5_qty double precision, breckdown5_value double precision, breakdown6_qty double precision, breckdown6_value double precision, breakdown7_qty double precision, breckdown7_value double precision) AS
    $BODY$
    BEGIN	
        Return Query
            Select 
                        GD.company_id, GD.company_name, GD.product_id, GD.product_code, GD.category_id, GD.category_name,
                        max(current_stock) as "total_stock", max(total_value)::double precision as "total_stock_value",
                ...
                Select 
                    ...
                    (
                        Select 
                row_number() over(partition by move.company_id, move.product_id order by move.company_id, move.product_id, move.date::date) row_id,
                move.company_id, move.company_name, move.date::date, move.product_id, move.product_code,
                move.category_id, move.category_name, quant.current_stock, quant.total_value, move.stock_value, move.stock_qty,move.stock_age
            from
            (
                select 
                    --row_number() over(partition by move.company_id, move.product_id order by move.company_id, move.product_id, move.date::date) row_id,
                ...
                Inner Join 
                (
                    Select q.company_id,q.product_id, round((sum(q.quantity) * max(COALESCE(prop.value_float,0)))::numeric,2) as total_value, sum(q.quantity) current_stock
                    from stock_quant q
                        inner join stock_location loc on loc.id = q.location_id
                        left join ir_property prop on prop.res_id = 'product.product,' || q.product_id and prop.name='standard_price'
                    where loc.usage = 'internal'
