...
class SetuStockMovementReport(models.TransientModel):
    ...
    def get_report_date_range(self):
        if self.get_report_from_beginning:
            ...
            end_date = self.upto_date
            return start_date, end_date
        else:
            start_date = self.start_date and self.start_date or '1900-01-01'
            end_date = self.end_date and self.end_date
            return start_date, end_date
    ...
    def get_file_name(self):
        ...
        if self.get_report_from_beginning:
            filename = filename + "_upto_" + self.upto_date + ".xlsx"
        else:
            filename = filename + "_from_" + self.start_date + "_to_" + self.end_date + ".xlsx"
        return filename
